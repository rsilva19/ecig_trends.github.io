...
A table showing the percentage of missingness in relevant variables
```{r}
#NA table
ecig_data %>%
  group_by(year) %>%
...
ecig_data %>%
  ...
  summarise(m = mean(quit, na.rm = TRUE)) %>%
  ggplot(aes(x = age, y = m)) +
  geom_col()
...
ecig_data %>%
  ...
  group_by(age) %>%
  summarise(prop_smokers = mean(current_cig, na.rm = TRUE)) %>%
